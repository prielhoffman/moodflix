...
- [ ] **1-minute flow**: Register → Top 10 recommendations → Like at least one (add to favorites)
...
| Favorites | `GET/POST /watchlist` with JWT; add by `show_id` (user-facing: Favorites) |
...
| **1-minute flow** | Home → Recommend requires explicit navigation; no "guest" path that skips registration | Unauthenticated users get recommendations but cannot add to favorites |
...
- **Favorites:** Like from results, view favorites, remove items (API: /watchlist)
